...
#posfile <- "MakeFinalFeatureTables/pos.features.txt"

##Negative (Class 0) Dataset File
#negfile <- "MakeFinalFeatureTables/neg.features.txt"

###########################
### Read in data, set rownames to gene ids and remove that col

positive <- read.table(posfile, header = T, sep = "\t", stringsAsFactors = FALSE)
rownames(positive) <- paste(positive$gene_id, seq(1, nrow(positive))) # to make the rownames unique since some genes have multiple TSSs
positive$gene_id <- NULL

negative <- read.table(negfile, header = T, sep = "\t", stringsAsFactors = FALSE)
rownames(negative) <- paste(negative$gene_id, seq(1, nrow(negative))) # to make the rownames unique since some genes have multiple TSSs
negative$gene_id <- NULL
...
train_positive <- positive[1:eighty_perc, ]
test_positive <- positive[(eighty_perc+1):nrow(positive),]
...
train_negative <- negative[1:eighty_perc, ]
test_negative <- negative[(eighty_perc+1):nrow(negative),]
...
all_train <- all_train[ , train_keep_logical]
